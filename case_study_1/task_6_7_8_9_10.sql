...
where (year(hd.ngay_lam_hop_dong) = 2020 ) and hd.ma_dich_vu not in (select distinct hd.ma_dich_vu from hop_dong hd where(year(hd.ngay_lam_hop_dong)=2021))
group by dv.ma_dich_vu;
...
select month(hd.ngay_lam_hop_dong) as thang, count(hd.ma_hop_dong) as so_luong
from hop_dong hd
